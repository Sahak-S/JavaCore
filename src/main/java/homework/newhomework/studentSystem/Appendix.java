package homework.newhomework.studentSystem;
...
public class Appendix {
    ...
    static String archivo = "C:\\Users\\User\\Desktop\\java\\java1\\git\\edge\\idea\\JAVA\\Документ Microsoft Word.docx";     // Create a String variable to store the name of the file to be read/written
                                               //   Ստեղծեք String փոփոխական՝ կարդալու/գրվող ֆայլի անունը պահելու համար

    static File f = new File(archivo);         // Create an instance of File using the fileName
                                               // Ստեղծեք File-ի օրինակ՝ օգտագործելով fileName-ը

    static ArrayList<Student> list = new ArrayList<>();         // The ArrayList will store String[] ( String arrays )
                                                                       // ArrayList-ը կպահի String[] (Տողային զանգվածներ)
    ...
    static BufferedReader br = new BufferedReader(new InputStreamReader(System.in));      // Declare BufferedWriter to get input from user
                                                                                          // Հայտարարեք BufferedWriter-ը՝ օգտվողից մուտքագրում ստանալու համար

    static boolean hey = false;                              //Static boolean     //  Ստատիկ բուլյան


    //==========================================================//
    //              MAIN METHOD      ՀԻՄՆԱԿԱՆ ՄԵԹՈԴ             //
    //==========================================================//

    public static void main(String[] args) {

        getting();    // placing all text content to the ArrayList to be handled  // ամբողջ տեքստային բովանդակությունը տեղադրելով ArrayList-ում, որը պետք է մշակվի

        displayMenu();  // to the  menu. //դեպի մենյու

    }     //END OF THE MAIN METHOD  // ՀԻՄՆԱԿԱՆ ՄԵԹՈԴԻ ԱՎԱՐՏ

    //======================================================================================================//
    // GETTING  METHOD // Method to read the file //   ՍՏԱՆԱԼՈՒ ՄԵԹՈԴ   //  Ֆայլը կարդալու մեթոդ           //
    //======================================================================================================//
    ...
    public static void getting() {
        ...
        FileReader leer = null;  // Create an instance of the FileReader using the File ( global )
                                 // Ստեղծեք FileReader-ի օրինակ՝ օգտագործելով Ֆայլը (գլոբալ)

        String[] data;          // Declare an array to store the data from each line of the file
                                // Հայտարարեք զանգված՝ ֆայլի յուրաքանչյուր տողից տվյալները պահելու համար
        try {

            leer = new FileReader(f);
        } catch (FileNotFoundException e) {
            System.out.println("\nError creating FileReader\n");
            System.out.println("\nՉհաջողվեց ստեղծել FileReader-ը\n");
        }

        BufferedReader in = new BufferedReader(leer);    // Create an instance of BufferedReader to read the file
                                                         //  Ստեղծեք BufferedReader-ի օրինակ՝ ֆայլը կարդալու համար

        String str = "";                              // Create variable to store each line of the file
                                                      // Ստեղծեք փոփոխական՝ ֆայլի յուրաքանչյուր տող պահելու համար
        try {

            str = in.readLine();                   // Read the first line  //  Կարդացեք առաջին տողը

            while (str != null) {                 // If the first line is not null keep reading the file
                                                  // Եթե  առաջին տողը զրոյական չէ, շարունակեք կարդալ ֆայլը

                data = str.split(";");   // split the line based on ";" and store the result into the data array
                                               // բաժանել տողը «;» -ի հիման վրա և արդյունքը պահել տվյալների զանգվածում

                Student s = new Student();    // Create an instance of the Student object
                                              // Ստեղծեք Student օբյեկտի օրինակ

                // Assign values for the attribute of the object   // Նշանակեք արժեքներ օբյեկտի հատկանիշի համար
                ...
                list.add(s);  // Stores the Student object into the list  // Պահպանում է Student օբյեկտը ցանկում

                str = in.readLine();      // Read the next line  // Կկարդա հաջորդ տողը

            }
        } catch (IOException ioe) {

            System.out.println("Error reading file");
            System.out.println("Ֆայլի ընթերցմն սխալ է");


        } catch (NumberFormatException nuE) {       //To catch the format errors   // Ձևաչափի սխալները բռնելու համար

            System.out.println("Error in format data file");
            System.out.println("Տվյալների ֆայլի ձևաչափի սխալ");
        }
        ...
        try {
            ...
            System.out.println("BufferedWriter-ը փակելիս սխալ առաջացավ");
        }
    }//END OF GETTING METHOD  // ՍՏԱՆԱԼՈՒ ՄԵԹՈԴԻ ԱՎԱՐՏ

    //==================================================================================================================================//
    // DISPLAY MENU  METHOD // Method to display the principal menu // ՄԵՆՅՈՒ ՄԵԹՈԴԻ ԴԻՍՊԼԵՅԸ  // ՀԻՄՆԱԿԱՆ ԸՆՏՐԵԼՈՒ ՑԱՆԿԸ               //
    //==================================================================================================================================//
    public static void displayMenu() {

        int option = 0;// Create variable to store the input from user

        welcomen();                //   ԲԱՐԻ ԳԱԼՈՒՍՏ

        do {

            //Menu options        ՄԵՆԻՈՒԻ ՑԱՆԿ
            System.out.println("\n----------------------------------------------\n");
            System.out.println("    \n     Menu\n    ՄԵՆԻՈՒ");
            System.out.println(" 1   Add new Student   Ավելացնել նոր ուսանող");
            System.out.println(" 2   Search for a Student  Փնտրել ուսանող");
            System.out.println(" 3   Display details of a Student  Ցուցադրել ուսանողի մանրամասները");
            System.out.println(" 4   Modify details of a Student  Փոփոխել ուսանողի մանրամասները");
            System.out.println(" 5   Delete a Student  Ջնջել ուսանողին");
            System.out.println(" 0   Exit  Ելք");
            System.out.println("\n----------------------------------------------\n");

            System.out.print(" Enter an option:   Մուտքագրեք տարբերակ");


            try {        //create a try and catch block to get user inputs and catch any error

                option = Integer.parseInt(br.readLine());  //Parsing the input from users          Օգտատերերի մուտքագրման վերլուծություն
            } catch (IOException ioe) {
    ...
    public static void addNewStudent() {
        ...
        do {//Loop until boolean is false
            ...
            try {
                ...
                //:::::::::::::::::::::::: Student number
                System.out.println("\n----------------------------------------------\n");
                System.out.println("\tEnter Student:");//Asking for Student Number              //  Ուսանողի համարի հարցնում
                System.out.println("\n----------------------------------------------\n");
                System.out.print("Number:       ");//Asking for Student Number                  Ուսանողի համարի հարցնում
